...
- ✅ [Phase 9 — The RenderNode Frontier](plans/phase-9-rendernode-frontier.md) — **complete** (IBL stretch not shipped, stays parked): `scene-material-lab` (our sphere, our flat/Gouraud/Phong/GGX shaders, lit by the engine's real sun), `hybrid-ray` (a fragment-shader ray tracer depth-composited against rasterized Zurich — reflections, shadow rays, bounce slider), `scene-path-trace` (an external progressive path tracer for an architectural proposal of three fixed-seed stacked-convex pavilions and their foundation; `<arcgis-daylight>` drives the shared geolocated sun, camera or daylight changes restart convergence, and ArcGIS depth-composites the rasterized terrain and surroundings against the traced proposal; serves path-tracing + global-illumination). Key engine finding: `opaque-color` injections get erased by the downstream resolve — custom passes must target `composite-color`
